...
`[図解が入ります: 図解タイトル: 30歳の入口までの道のり: 図解の詳細な説明: Webエンジニアとしての日常から、展示会向けデモアプリ開発の担当へ、仕様のないコードの読解、深夜の試行錯誤を経て、iOS開発への関心に至るまでの流れを時系列で示す。一直線の矢印ではなく、迷いや行き詰まりを表す曲がりくねった経路として描き、きれいな道ではなかったことを視覚的に表現する]`
...
`[図解が入ります: 図解タイトル: 過去の経験と現在の接続マップ: 図解の詳細な説明: 左側にWebエンジニア時代に得たもの（運用保守の感覚、業務要求の変化への耐性、バックエンドの理解、仕様の曖昧さへの警戒心、顧客対応の経験）を並べ、右側にモバイル開発で実際に効いた場面（本番コードへの慎重さ、要求整理、API連携のトラブル対応、仕様確認の勘どころ）を並べる。両者を線で結んで対応関係を可視化しつつ、一部の線は点線にして「まだ接続先が見つかっていない経験」として余白を残す]`
...
`[図解が入ります: 図解タイトル: 技術力とは別軸の武器: 図解の詳細な説明: 縦軸を技術力、横軸を業務経験・対人スキル・言語化力とした二軸のマップ。20代から技術一本で伸びてきた人のベクトルと、30代から別領域を経由して入ってきた人のベクトルが異なる方向に伸びている様子を描く。どちらが優れているという比較ではなく、持ち物の種類が違うということを視覚的に示す]`
...
`[図解が入ります: 図解タイトル: 未来ある若手に残せるもの: 図解の詳細な説明: 中心に「未来ある若手に残したいものは何か」という問いを配置し、そこから放射状に「知見の言語化」「越境してきた記録」「失敗を学びに変える姿勢」「若手から学ぶ姿勢」「不格好でも続けてきた記録」といった要素を伸ばすマインドマップ。一部のノードは空欄のまま残し、読者自身が自分の答えを書き込める余白を設ける]`
